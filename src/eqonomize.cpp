...
void Eqonomize::openSynchronizationSettings() {
	...
	box1->addLayout(grid);
	QDialogButtonBox *buttonBox = new QDialogButtonBox(QDialogButtonBox::Cancel | QDialogButtonBox::Ok, Qt::Horizontal, syncDialog);
	buttonBox->button(QDialogButtonBox::Ok)->setDefault(true);
	buttonBox->button(QDialogButtonBox::Cancel)->setAutoDefault(false);
	buttonBox->button(QDialogButtonBox::Ok)->setShortcut(Qt::CTRL | Qt::Key_Return);
...
void Eqonomize::accountMoved(QTreeWidgetItem *i, QTreeWidgetItem *target) {
	...
	if(account->type() == ACCOUNT_TYPE_EXPENSES || account->type() == ACCOUNT_TYPE_INCOMES) {
		...
		if(find_account) {
			QMessageBox::critical(this, tr("Error"), account->type() == ACCOUNT_TYPE_EXPENSES ? tr("The entered name is used by another expense category.") : tr("The entered name is used by another income category."));
			return;
		}
		QTreeWidgetItem *prev_parent_item = i->parent();
